...
class Enemy(Tank):
    ...
    def __init__(self, level, side, position=None, direction=None, filename=None):
        ...
        images = [
            ...
            gl.sprites.subsurface(32 * 2, 16 * 2, 13 * 2, 15 * 2),
            gl.sprites.subsurface(48 * 2, 16 * 2, 13 * 2, 15 * 2),
            gl.sprites.subsurface(64 * 2, 16 * 2, 13 * 2, 15 * 2),
            gl.sprites.subsurface(80 * 2, 16 * 2, 13 * 2, 15 * 2)
        ]
        ...
        self.timer_uuid_fire = gl.gtimer.add(1000, lambda: self.fire())

        # turn on flashing
        if self.bonus:
            self.timer_uuid_flash = gl.gtimer.add(200, lambda: self.toggle_flash())

    def toggle_flash(self):
        """ Toggle flash state """
        if self.state not in (self.STATE_ALIVE, self.STATE_SPAWNING):
            gl.gtimer.destroy(self.timer_uuid_flash)
            return
    ...
    def spawn_bonus(self):
        ...
        gl.bonuses.append(bonus)
        gl.gtimer.add(500, lambda: bonus.toggle_visibility())
        gl.gtimer.add(10000, lambda: gl.bonuses.remove(bonus), 1)

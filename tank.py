...
class Tank(object):
    ...
    def __init__(self, level, side, position=None, direction=None, filename=None):
        ...
        self.shield_images = [
            gl.sprites.subsurface(0, 48 * 2, 16 * 2, 16 * 2),
            gl.sprites.subsurface(16 * 2, 48 * 2, 16 * 2, 16 * 2)
        ]
        self.shield_image = self.shield_images[0]
        self.shield_index = 0

        self.spawn_images = [
            gl.sprites.subsurface(32 * 2, 48 * 2, 16 * 2, 16 * 2),
            gl.sprites.subsurface(48 * 2, 48 * 2, 16 * 2, 16 * 2)
        ]
        ...
        # spawning animation
        self.timer_uuid_spawn = gl.gtimer.add(100, lambda: self.toggle_spawn_image())

        # duration of spawning
        self.timer_uuid_spawn_end = gl.gtimer.add(1000, lambda: self.end_spawning())
    ...
    def toggle_shield_image(self):
        ...
        if self.state != self.STATE_ALIVE:
            gl.gtimer.destroy(self.timer_uuid_shield)
            return
        if self.shielded:
            self.shield_index += 1
    ...
    def draw(self):
        ...
        if self.state == self.STATE_ALIVE:
            ...
            if self.shielded:
                gl.screen.blit(self.shield_image, [self.rect.left - 3, self.rect.top - 3])
        elif self.state == self.STATE_EXPLODING:
            self.explosion.draw()
        elif self.state == self.STATE_SPAWNING:
            gl.screen.blit(self.spawn_image, self.rect.topleft)
    ...
    def fire(self, forced=False):
        ...
        if self.state != self.STATE_ALIVE:
            gl.gtimer.destroy(self.timer_uuid_fire)
            return False
    ...
    def bullet_impact(self, friendly_fire=False, damage=100, tank=None):
        ...
        if not friendly_fire:
            ...
            if self.health < 1:
                if self.side == self.SIDE_ENEMY:
                    ...
                    tank.score += points
                    if gl.play_sounds:
                        gl.sounds["explosion"].play()

                    gl.labels.append(Label(self.rect.topleft, str(points), 500))
        ...
        if self.side == self.SIDE_ENEMY:
            ...
            if not self.paralised:
                self.set_paralised(True)
                self.timer_uuid_paralise = gl.gtimer.add(10000, lambda: self.set_paralised(False), 1)
            return True

    def set_paralised(self, paralised=True):
        """ set tank paralise state
        @param boolean paralised:
        @return None
        """
        if self.state != self.STATE_ALIVE:
            gl.gtimer.destroy(self.timer_uuid_paralise)
            return
